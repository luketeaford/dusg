  ...
  return Promise.all(siteObj.map(
    async (fileObj, anIndex, siteInfo) => {
      const pageObject = {
        ...
        [filesKey]: siteInfo,
        [pathKey]: fileObj[pathKey],
        [inputPathKey]: fileObj[inputPathKey]
      }

      const path = fileObj[pathKey]

      if (!pageObject[htmlKey]) return

      const outputDir = path.slice(0, path.lastIndexOf('/'))
      await fs.mkdir(outputDir, { recursive: true })

      const templateResult = template({ ...pageObject })
