...
const makeMapToNewPath = (src, dest, rename, extension) => x => {
...
const build = async function (settings) {
  ...
  const rename = cleanUrls !== false ? '/index' : ''
  const extensionToUse = extension || '.html'

  const mapToOutputPath = makeMapToNewPath(src, dest, rename, extensionToUse)
