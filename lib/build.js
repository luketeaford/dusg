...
const build = async function (settings) {
  const { src, dest, cleanUrls, extension, template } = settings
  ...
  const templateFn = compose(template, parse)
  ...
  return Promise.all(sourceFiles.map(async function (aFilename, anIndex) {
    ...
    const finalData = templateFn(data.toString())
    if (!finalData) return
    await fs.mkdir(outputDir, { recursive: true })
    return fs.writeFile(outputPath, finalData)
  }))
}
